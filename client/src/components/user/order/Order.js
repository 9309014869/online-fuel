...
function Order(){
    ...
    const renderedStations = stations ? stations.map((element) => {
      // Check if element.location is defined before accessing its properties
      if (element.location && typeof element.location.lat === 'number' && typeof element.location.lng === 'number') {
          ...
          const station = {
              ...element, // Spread the properties of element
              distance
          };
  
          return (
              <ListStation station={station} key={element.id} />
          );
      } else {
          // Handle case where element.location is undefined or lat/lng are not numbers
